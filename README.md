...
**Этап 2 (в работе): веб-фронтенд.** Готов дашборд только для просмотра: боксы и слоты, компоненты, калибровки,
журнал событий; обновляется сам раз в секунду.

**Дальше:** запуск калибровки и прочие действия из фронта, миграции БД (Alembic), авторизация брокера, прошивка ESP по протоколу ниже.
...
- `src/components/` — `sku-app` (страница), `box-card`, `locker-tile` (подсвечивается при изменении),
...
rm -rf backend/var emulator/var                   # сбросить БД и идентичности боксов
```
...
### Сценарий калибровки через API
N штук задаётся при запуске калибровки в бэкенде, M граммов — в эмуляторе при засыпке; бокс считает вес штуки = M / N.
```bash
...
- `POST /api/calibrations`, `GET /api/calibrations?status=`
