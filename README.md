...
На карточке бокса есть «Удалить бокс», на карточке слота — «Удалить слот». Оба действия требуют подтверждения.
Удалённые боксы и слоты исчезают из склада; новая телеметрия не создаёт их заново. История событий и сведения
о компонентах по NFC сохраняются. Удаление бокса также скрывает все его слоты, а активные заявки на калибровку
отменяются. Работающее устройство после удаления продолжит передавать MQTT-сообщения, но backend их игнорирует.

API: `DELETE /api/boxes/{box_id}` и `DELETE /api/boxes/{box_id}/lockers/{locker_id}`. Для защиты от повторного
появления используются таблицы `deleted_boxes` и `deleted_lockers`; их создаёт backend при запуске.
Стартовый `emu-box-001` удалён из конфигурации эмулятора. Новые виртуальные боксы можно добавлять через его вкладку.
