...
«⚠ нет нуля» (и счётчиком в шапке бокса), вместо калибровки у него кнопка «Установить ноль» — активна, только когда
ячейка вставлена (`nfc_flag`). Пока ноля нет, бэкенд не пишет событий учёта по слоту и не даёт его калибровать;
первое показание с нулём засчитывается как `cell_inserted`. Пустую неоткалиброванную ячейку можно переобнулить
той же кнопкой.
